...
export namespace UI
{
    ...
    struct Settings {
        ...
        float light2Pos[3] = {1.0f, 1.0f, 1.0f}; 
    ...
    public:
        ...
        void init(Render::VulkanContext* context) {
            ...
            check(vkCreateDescriptorPool(ctx->device, &pool_info, nullptr, &imguiPool) == VK_SUCCESS, "Could not create ImGui descriptor pool");
            ...
            ImGui_ImplVulkan_InitInfo init_info = {
                ...
                .Device = ctx->device,
                .QueueFamily = ctx->computeQueueFamilyIndex,
                .Queue = ctx->computeQueue,
        ...
        void render(VkCommandBuffer cb, uint32_t imageIndex) {
...

            {
                ...
                if (ImGui::CollapsingHeader("Scene Control", ImGuiTreeNodeFlags_DefaultOpen)) {
                    ...
                    ImGui::Checkbox("Invert Y Axis (Mirror)", &settings.invertY);
    ...
    private:
        Render::VulkanContext* ctx = nullptr;
        ...
        void cleanup() {
            ...
            if (imguiPool != VK_NULL_HANDLE) vkDestroyDescriptorPool(ctx->device, imguiPool, nullptr);
        ...
        void create_render_pass() {
            ...
            check(vkCreateRenderPass(ctx->device, &info, nullptr, &renderPass) == VK_SUCCESS, "Could not create ImGui render pass");
        }

        void create_framebuffers() {
            framebuffers.resize(ctx->swapChainImageViews.size());
            for (size_t i = 0; i < ctx->swapChainImageViews.size(); i++) {
                VkImageView attachment[] = { ctx->swapChainImageViews[i] };
                ...
                check(vkCreateFramebuffer(ctx->device, &info, nullptr, &framebuffers[i]) == VK_SUCCESS, "Could not create ImGui framebuffer");
            }
        }
    };
}